...
function DaemonInterface(daemons, logger){
    ...
    function performHttpRequest(instance, jsonData, callback){
        ...
        var req = http.request(options, function(res) {
            ...
            // Append response data chunks to array
            res.on('data', function (chunk) {
                chunks.push(chunk);
            });

            // Join chunks and process response
            res.on('end', function() {
                ...
                switch (encoding) {
                    case 'gzip':
                        zlib.gunzip(input, function(err, buf) {
                            parseJson(res, buf.toString('utf8'));
                        });
                    break;
                    case 'deflate':
                    ...
                    break;
                    default: {
                        // Default behaviour: no compression
                        parseJson(res, input.toString('utf8'));
                    }
                }
            });
        });
