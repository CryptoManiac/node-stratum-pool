...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    function SubmitAuxBlock(aux, headerBuffer, coinbaseBuffer, blockHash, callback) {
        var branchProof = _this.jobManager.auxMerkleTree.getHashProof(util.uint256BufferFromHash(aux.rpcData.hash));
        ...
        aux.daemon.cmd('getauxblock',
            [aux.rpcData.hash, auxpow.toString('hex')],
            function(result) {
                if(result[0].error) {
                    emitErrorLog('Failed to submit potential block: ' + result.error);
                }
                else {
    ...
    function SetupJobManager(){
        ...
        _this.jobManager.on('newBlock', function(blockTemplate){
            ...
            var auxResult = function(hash, aux) {
                if(!hash) return;
                CheckBlockAccepted(hash, aux.daemon, function(accepted) {
            ...
            for(var i = 0;i < _this.auxes.length;i++) {
                var aux = _this.auxes[i];
                if(aux.rpcData.target.ge(shareData.blockBigNum)) {
                    SubmitAuxBlock(aux, new Buffer(blockHexInvalid, 'hex').slice(0,80), shareData.coinbaseBuffer, shareData.headerHash, auxResult);
                }
            }
