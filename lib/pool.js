...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    this.options.auxes = options.auxes || [];
    ...
    function SubmitAuxBlock(aux, headerBuffer, coinbaseBuffer, blockHash, callback) {
        ...
        _this.auxes[aux].daemon.cmd('getauxblock',
            ...
            function(result) {
                if(result[0].error) {
                    emitErrorLog('Failed to submit auxillery potential block: ' + result.error);
                }
                else {
                    if(!result[0].response) {
                        emitWarningLog('Submitted block was rejected by the ' + options.auxes[aux].name + ' network! Check its log for more information');
                    }
                }
        ...
        _this.jobManager.on('newBlock', function(blockTemplate){
            ...
            var auxResult = function(hash, aux) {
                ...
                CheckBlockAccepted(hash, _this.auxes[aux].daemon, function(accepted, tx, height, value) {
                    if(accepted) {
                        _this.emit('auxblock', options.auxes[aux].symbol, height, hash, tx, shareData.difficulty, shareData.worker);
                    }
                    else {
                        emitErrorLog('Auxillary block was not detected to have been accepted by ' + options.auxes[aux].name + ' network: ' + hash);
                    }
                    UpdateAuxes();
        ...
        _this.daemon.cmd('getblocktemplate',
            ...
            function(result){
                if (result.error){
                    ...
                    data.auxes = [];
                    for(var i = 0; i < _this.auxes.length; i++) {
                        data.auxes.push(_this.auxes[i].rpcData);
                    }
            ...
            function(result){
                if (result.error){
                    ...
                    if(_this.auxes[index].rpcData) {
                        if(_this.auxes[index].rpcData.hash != result.response.hash) {
                            _this.updateNeeded = true;
                        }
                    }
