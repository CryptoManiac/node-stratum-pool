        ...
        _this.jobManager.on('newBlock', function(blockTemplate){
            ...
            var auxResult = function(hash, aux) {
                ...
                CheckBlockAccepted(hash, _this.auxes[aux].daemon, function(accepted, tx, height) {
                    if(!accepted) emitErrorLog('Block was not detected to have been accepted by' + aux.name + ' network: ' + hash);
            ...
            if (!isValidBlock)
                ...
                SubmitBlock(blockHex, function(){
                    CheckBlockAccepted(shareData.blockHash, _this.daemon, function(isAccepted, tx){
                        ...
                        _this.emit('block', options.coin.symbol, shareData.blockHash);
