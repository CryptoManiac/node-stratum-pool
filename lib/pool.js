        ...
        _this.jobManager.on('newBlock', function(blockTemplate){
            ...
            var auxResult = function(hash, aux) {
                ...
                CheckBlockAccepted(hash, _this.auxes[aux].daemon, function(accepted, tx, height, value) {
                    if(accepted) {
                        _this.emit('block', options.auxes[aux].symbol, height, hash, tx, -1, shareData.difficulty, shareData.worker);
                    }
                    else {
                        emitErrorLog('Block was not detected to have been accepted by ' + options.auxes[aux].name + ' network: ' + hash);
                    }
                    UpdateAuxes();
